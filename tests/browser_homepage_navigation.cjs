...
(async () => {
  ...
  try {
    for (const locale of ['lv','en']) for (const width of [320,390,768,1440]) {
      ...
      assert.deepEqual(await links.evaluateAll(nodes => nodes.map(node => node.getAttribute('href'))), ['#how-it-works','tutorial','tutorial#faq-safety','#legal',`https://lawyer-wishes-parent-console.trycloudflare.com/${locale}/transport`]);
      ...
      if (width < 768) {
        ...
        await toggle.click();
        await page.route('https://lawyer-wishes-parent-console.trycloudflare.com/**', route => route.fulfill({contentType:'text/html', body:'<title>External service</title>'}));
